...
Score::Score(AdvancedPlayer * advPlayer, int placedX, int placedY) {   
    // Declare Variables
    this->advPlayer = advPlayer;
    this->grid = advPlayer->getGrid();
...
void Score::setAdvancedNext(Direction direction) {
    //Set Variables
    nextX = currentX;
    nextY = currentY;
    ...
    // Check Right Direction
    } else if (direction == 1 && currentX != ADV_MAX_RIGHT) {
        nextX = currentX + 1;

    // Check Downwards Direction
    } else if (direction == 2 && currentY != ADV_MAX_DOWN) {
        nextY = currentY + 1;
